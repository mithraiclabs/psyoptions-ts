...
export const getFarmLps = async (
  ...
  return decodedFarmLedgers.filter(farmLedger => minimumFarmAmount.lt(farmLedger.deposited))
};
 ...
 * @returns 
 */
export const getOwnersFromFarmLedgers = (decodedFarmLedgers: FarmLedger[]): PublicKey[] => decodedFarmLedgers.map(x => x.owner);
